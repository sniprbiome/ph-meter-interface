...
+ *Dose multiplier interval*: An pH interval in the form of a single number, that decides whether the associated pump should pump multiple times when the actual pH is less than the expected pH. If the interval value is 0.5, the expected pH is 6 and the actual pH is 5.1, two doses would be dispensed. If the interval instead was 0.3, four doses would be dispensed.
